...
void expansion_circle() {
	...
	*/
	...
	while (System::Update())
	{
		Circle{ 150, 200, 40 }.draw(ColorF{ 0.8 });

		Circle{ Vec2{ 400, 200 }, 80 }.draw(ColorF{ 1.0 }, ColorF{ 1.0, 0.6, 0.4 });

		Cursor::Pos().asCircle(120).draw(ColorF{ 0.4 });

		//円の枠線
		Circle{ 150, 450, 100 }.drawFrame(10, Palette::Seagreen);

		Circle{ 400, 450, 100 }.draw().drawFrame(2, 8, Palette::Seagreen);

		Circle{ 650, 450, 100 }.drawFrame(20, ColorF{ 0.0, 0.0 }, ColorF{ 0.0, 1.0 });
	}
}
//扇形
void fanshape() {
	...
	*/
	...
	while (System::Update())
	{
		...
		Circle{ 250, 150, 60 }.drawPie(120_deg, 240_deg, ColorF{ 0.1, 0.3, 0.1 }, ColorF{ 0.3, 1.0, 0.6 });

		Circle{ 650, 150, 60 }.drawPie(0_deg, 240_deg, ColorF{ 0.1, 0.3, 0.1 }, ColorF{ 0.3, 1.0, 0.6 });
	}
}
//弧
void arc() {
...
//長方形
void expansion_rectangle(){
	/*
	Rect{ 幅, 高さ }
	RectF{ 幅, 高さ }									左上が (0, 0) の長方形を作成します
	Rect{ 幅と高さ }		
	RectF{ 幅と高さ }									左上が (0, 0) の長方形を作成します
	Rect{ 一辺の長さ }
	RectF{ 一辺の長さ }									左上が (0, 0) の正方形を作成します
	Rect{ 左上の X 座標, 左上の Y 座標, 幅, 高さ }
	RectF{ 左上の X 座標, 左上の Y 座標, 幅, 高さ }		長方形を作成します
	Rect{ 左上の X 座標, 左上の Y 座標, 幅と高さ }
	RectF{ 左上の X 座標, 左上の Y 座標, 幅と高さ }		長方形を作成します
	Rect{ 左上の X 座標, 左上の Y 座標, 一辺の長さ }
	RectF{ 左上の X 座標, 左上の Y 座標, 一辺の長さ }	正方形を作成します
	Rect{ 左上の座標, 幅, 高さ }
	RectF{ 左上の座標, 幅, 高さ }						長方形を作成します
	Rect{ 左上の座標, 幅と高さ }
	RectF{ 左上の座標, 幅と高さ }						長方形を作成します
	Rect{ 左上の座標, 一辺の長さ }
	RectF{ 左上の座標, 一辺の長さ }						正方形を作成します
	Rect{ Arg::center(中心座標), 幅, 高さ }
	RectF{ Arg::center(中心座標), 幅, 高さ }			中心座標を指定して長方形を作成します
	Rect{ Arg::center(中心座標), 幅と高さ }
	RectF{ Arg::center(中心座標), 幅と高さ }			中心座標を指定して長方形を作成します
	Rect{ Arg::center(中心座標), 一辺の長さ }
	RectF{ Arg::center(中心座標), 一辺の長さ }			中心座標を指定して正方形を作成します
	Rect::FromPoints(角の座標, その対角線上の角の座標)
	RectF::FromPoints(角の座標, その対角線上の角の座標)	与えられた 2 点から長方形を作成します。サイズが正の値である有効な長方形が作られます
...
void rounded_rectangle() {
	...
	*/
	...
	const Rect rect{ 10, 500, 100, 80 };

	while (System::Update())
	{
		RoundRect{ 10, 10, 150, 50, 20 }.draw();

		RoundRect{ Arg::center(650, 100), 10, 40, 10 }.draw();
...
void line_segment() {
	...
	rect.top(), rect.bottom(), rect.left(), rect.right()				長方形（Rect または RectF）の上・下・左・右の辺を表す線分を作成します
...
void arrow() {
	...
	*/
	...
	while (System::Update())
	{
		...
		.drawArrow(10, SizeF{ 40, 80 }, ColorF{ 1.2 });

		Line{ 600, 100, 700, 400 }
		.drawDoubleHeadedArrow(8, SizeF{ 30, 30 }, ColorF{ 0.6 });
	}
}
//三角形
void triangle() {
	...
	Triangle::FromPoints(pos0, pos1, pos2)							3 つの頂点座標を指定して三角形を作成します。
	有効な三角形が作られるように頂点の順序が調整されます
	*****************************
...
void convex_quadrangle() {
	...
	*/
	...
	while (System::Update())
	{
		Quad{ Vec2{ 30, 30 }, Vec2{ 45, 30 }, Vec2{ 90, 90 }, Vec2{ 30, 90 } }.draw(ColorF{ 0.2 }, ColorF{ 0.6 }, ColorF{ 0.8 }, ColorF{ 1.0 });
...
void ellipse() {
	/*
	llipse{ 中心の X 座標, 中心の Y 座標, X 軸方向の半径, Y 軸方向の半径 }	楕円を作成します
	...
	*/
	...
	while (System::Update())
	{
		...
		Ellipse{ 300, 400, 50, 50 }.draw(ColorF{ 0.1 });
	}
}
//その他の図形
void other_shapes() {
...
void Main()
{
	...
	while (System::Update())
	{
		...
		///* 弧 */arc();
		///* 弓形 */arched();
		///* 長方形 */expansion_rectangle();
		///* 長方形の枠 */rectangular_frame();
		///* 角丸長方形 */rounded_rectangle();
		///* 線分 */line_segment();
		///* 矢印 */arrow();
		///* 三角形 */triangle();
		///* 凸な四角形 */convex_quadrangle();
		///* 長方形のスライド・傾斜 */shear_and_skew();
		///* 楕円 */ellipse();
		///* その他の図形 */other_shapes();
		///* 自由な多角形 */polygon();
		///* 連続する線分 */expansion_line_segment();
		///* スプライン曲線 */spline_curve();
		///* ベジェ曲線 */bezier_curve();
		///* 図形の影 */figure_shadow();
		/* Polygon のワイヤフレーム表示 */polygon_wireframe();
	}
}
